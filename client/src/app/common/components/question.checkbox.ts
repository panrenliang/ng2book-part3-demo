...
@Component({
  ...
  <template [ngIf] = "isPublished">
     ...
     <p *ngFor="#item of options">
        <input name="group2" type="checkbox" id="{{item.key}}"  />
  ...
  <template [ngIf]="!isPublished && !isEdit">
    <p>{{desc}}</p>
     <p *ngFor="#item of options">
        <input name="group1" disabled="disabled" type="checkbox" id="{{item.key}}"  />
        <label attr.for="{{item.key}}">{{item.value}}</label>
     </p>
...
export class QuestionCheckboxCmp extends QuestionComponent {
  options:any[];
  key:number;
